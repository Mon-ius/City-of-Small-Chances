...
SRC="$ROOT/tools/gen/source_batch60"
...
notes = {
    "PROP_Bird_Cormorant": "reads as a black-bronze cormorant perched with wings half-spread to dry, no legible text",
    "PROP_Bird_Heron": "reads as a tall blue-grey heron standing hunched with a dagger bill, no legible text",
    "PROP_Bird_Ducks": "reads as a low raft of three mallard ducks on the waterline, no legible text",
}
...
for stem in stems:
    ...
    aspect = w / h
    if stem == "PROP_Bird_Cormorant" and not (0.7 <= aspect <= 1.55):
        raise SystemExit(f"{stem}: expected square-ish aspect, got {w}x{h}")
    if stem == "PROP_Bird_Heron" and aspect >= 0.95:
        raise SystemExit(f"{stem}: expected portrait aspect, got {w}x{h}")
    if stem == "PROP_Bird_Ducks" and aspect <= 1.2:
