...
SRC="$ROOT/tools/gen/source_batch7"
TMP="${TMPDIR:-/tmp}/batch7_postprocess"
MAGICK="${MAGICK:-magick}"

mkdir -p \
  "$ROOT/assets/textures/interiors" \
  "$ROOT/assets/textures/workplaces" \
  "$TMP"

for src in \
  ENV_Interior_Shelter_albedo.png \
  ENV_Interior_SharedRoom_albedo.png \
  ENV_Interior_Studio_albedo.png \
  ENV_Interior_Apartment_albedo.png \
  ENV_Interior_LiveWork_albedo.png \
  ENV_Work_Warehouse_albedo.png \
  ENV_Work_Kitchen_albedo.png \
  ENV_Work_Civic_albedo.png
do
  if [[ ! -f "$SRC/$src" ]]; then
...
SURFACES = [
    ("ENV_Interior_Shelter", "interiors", 3.0, 220, 0, 52, "rough"),
    ("ENV_Interior_SharedRoom", "interiors", 2.4, 200, 10, 50, "vinyl"),
    ("ENV_Interior_Studio", "interiors", 2.2, 170, 0, 46, "soft"),
    ("ENV_Interior_Apartment", "interiors", 2.6, 150, 0, 44, "wood"),
    ("ENV_Interior_LiveWork", "interiors", 2.0, 110, 25, 42, "polished"),
    ("ENV_Work_Warehouse", "workplaces", 3.2, 190, 15, 52, "rough"),
    ("ENV_Work_Kitchen", "workplaces", 3.6, 90, 10, 38, "tile"),
    ("ENV_Work_Civic", "workplaces", 1.8, 150, 20, 40, "speckle"),
]
...
def tuned_height(albedo, kind):
    ...
    if kind == "soft":
        height = height.filter(ImageFilter.GaussianBlur(0.2))
    return height
...
filelist = tmp / "batch7_files.txt"
checks = tmp / "batch7_edge_rms_prequant.txt"

with filelist.open("w", encoding="utf-8") as files, checks.open("w", encoding="utf-8") as check_out:
    for stem, folder, strength, roughness, metalness, band, kind in SURFACES:
        out = root / "assets" / "textures" / folder
...
while IFS= read -r f; do
  ...
  cp "$optimized" "$f"
done < "$TMP/batch7_files.txt"

python3 - "$TMP/batch7_files.txt" <<'PY'
from pathlib import Path
...
python3 - "$TMP/batch7_files.txt" "$TMP/batch7_edge_rms_final.txt" <<'PY'
from pathlib import Path
from PIL import Image
import math
import sys
...
for path in files:
    if path.name.endswith("_albedo.png"):
        stem = path.name.removesuffix("_albedo.png")
        lines.append(f"{stem} edge_rms={edge_rms(Image.open(path)):.3f}")
...
identify -format '%d/%f %wx%h %b\n' \
  "$ROOT/assets/textures/interiors"/ENV_Interior_{Shelter,SharedRoom,Studio,Apartment,LiveWork}_{albedo,normal,orm}.png \
  "$ROOT/assets/textures/workplaces"/ENV_Work_{Warehouse,Kitchen,Civic}_{albedo,normal,orm}.png
