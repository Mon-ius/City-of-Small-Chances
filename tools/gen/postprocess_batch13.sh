...
SRC="$ROOT/tools/gen/source_batch13"
TMP="${TMPDIR:-/tmp}/batch13_postprocess"
MAGICK="${MAGICK:-magick}"

mkdir -p \
  "$ROOT/assets/textures/workplaces" \
  "$ROOT/assets/textures/training" \
  "$TMP"

for src in \
  ENV_Work_Clinic_albedo.png \
  ENV_Work_Repair_albedo.png \
  ENV_Work_Laundry_albedo.png \
  ENV_Train_CourseRoom_albedo.png \
  ENV_Train_Community_albedo.png
do
  if [[ ! -f "$SRC/$src" ]]; then
...
SURFACES = [
    ("ENV_Work_Clinic", "workplaces", 2.0, 120, 10, 42, "soft"),
    ("ENV_Work_Repair", "workplaces", 3.4, 200, 20, 54, "wood"),
    ("ENV_Work_Laundry", "workplaces", 2.2, 95, 15, 38, "tile"),
    ("ENV_Train_CourseRoom", "training", 2.4, 150, 5, 42, "soft"),
    ("ENV_Train_Community", "training", 2.8, 185, 5, 50, "vinyl"),
]
...
def tuned_height(albedo, kind):
    ...
    if kind in {"tile", "vinyl"}:
        height = height.filter(ImageFilter.UnsharpMask(radius=1.0, percent=110, threshold=2))
    if kind == "wood":
        height = height.filter(ImageFilter.UnsharpMask(radius=0.9, percent=120, threshold=2))
    if kind == "soft":
...
filelist = tmp / "batch13_files.txt"
checks = tmp / "batch13_edge_rms_prequant.txt"
...
while IFS= read -r f; do
  ...
  cp "$optimized" "$f"
done < "$TMP/batch13_files.txt"

python3 - "$TMP/batch13_files.txt" <<'PY'
from pathlib import Path
...
python3 - "$TMP/batch13_files.txt" "$TMP/batch13_edge_rms_final.txt" <<'PY'
from pathlib import Path
from PIL import Image
import math
import sys
...
lines = []
for path in files:
    lines.append(f"{path.name} edge_rms_final={edge_rms(Image.open(path)):.3f}")
out.write_text("\n".join(lines) + "\n", encoding="utf-8")
print(out.read_text(encoding="utf-8"), end="")
PY

identify -format '%d/%f %wx%h %b\n' \
  "$ROOT/assets/textures/workplaces"/ENV_Work_Clinic_*.png \
  "$ROOT/assets/textures/workplaces"/ENV_Work_Repair_*.png \
  "$ROOT/assets/textures/workplaces"/ENV_Work_Laundry_*.png \
  "$ROOT/assets/textures/training"/ENV_Train_CourseRoom_*.png \
  "$ROOT/assets/textures/training"/ENV_Train_Community_*.png
