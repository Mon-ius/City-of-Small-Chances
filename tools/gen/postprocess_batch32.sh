...
TMP="${TMPDIR:-/tmp}/harbour_batch32_postprocess"
SOURCE_DIR="${SOURCE_DIR:-${1:-$ROOT/tools/gen/source_batch32}}"
MAGICK="${MAGICK:-magick}"
CHROMA="${CODEX_HOME:-$HOME/.codex}/skills/.system/imagegen/scripts/remove_chroma_key.py"

NAMES=(Fisherman Nun Veteran FlowerGirl Dockmaster Sweep)
TARGET_FEET_ROW=976
...
if ! command -v "$MAGICK" >/dev/null 2>&1; then
  echo "Missing ImageMagick command: $MAGICK" >&2
  exit 1
fi

key_for() {
  case "$1" in
    FlowerGirl) printf '#ff00ff' ;;
    *) printf '#00ff00' ;;
  esac
}
...
for y in range(im.height):
    for x in range(im.width):
        ...
        if a == 0:
            px[x, y] = (0, 0, 0, 0)
            continue
        near_green = g >= 145 and r <= 125 and b <= 125 and (g - max(r, b)) >= 42
        near_magenta = r >= 145 and b >= 145 and g <= 125 and (min(r, b) - g) >= 42
        if near_green or near_magenta:
            if a < 252:
                px[x, y] = (0, 0, 0, 0)
                removed += 1
            else:
                # Defensive final despill: preserve value while eliminating pure key hue.
...
canvas.save(out)
print(f"{name}: edge cleanup removed={removed}, despilled={desaturated}, scale={scale:.3f}, placed={nw}x{nh}@{x},{y}")
PY
...
names = ["Fisherman", "Nun", "Veteran", "FlowerGirl", "Dockmaster", "Sweep"]
notes = {
    "Fisherman": "yellow oilskin fisherman with sou'wester, boots, creel/net; no legible text",
    "Nun": "black habit and white wimple charity nun with small rosary; no legible text",
    "Veteran": "faded greatcoat veteran with pinned sleeve and crutch; no legible text",
    "FlowerGirl": "shawled young flower seller with basket of cut flowers; no legible text",
    "Dockmaster": "navy peaked-cap harbour official with ledger and spyglass; no legible text",
    "Sweep": "soot-blackened chimney sweep with shoulder brushes/rods; no legible text",
}
failed = False
tiles = []
...
for name in names:
    ...
    if im.size != (512, 1024):
        ...
        failed = True
    pix = im.load()
    corners = [pix[0, 0][3], pix[511, 0][3], pix[0, 1023][3], pix[511, 1023][3]]
    ...
    if green_fringe or magenta_fringe:
        print(f"{path.name}: chroma fringe green={green_fringe} magenta={magenta_fringe}", file=sys.stderr)
        failed = True
    ...
    print(
        ...
        f"greyscale_luma={luma:.1f}; note={notes[name]}"
    )

sheet = Image.new("RGB", (180 * 6, 360), (70, 70, 70))
for i, tile in enumerate(tiles):
    sheet.paste(tile, (i * 180, 0))
sheet_path = tmp / "batch32_grey_check.png"
...
"$MAGICK" "$OUT"/CHAR_Harbour_Citizen_{Fisherman,Nun,Veteran,FlowerGirl,Dockmaster,Sweep}_albedo.png \
  -background '#808080' -alpha remove -alpha off +append "$TMP/batch32_strip_on_grey.png"

identify -format '%f %wx%h %b\n' "$OUT"/CHAR_Harbour_Citizen_{Fisherman,Nun,Veteran,FlowerGirl,Dockmaster,Sweep}_albedo.png
echo "Grey strip check: $TMP/batch32_strip_on_grey.png"
