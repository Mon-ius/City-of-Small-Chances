...
SRC="$ROOT/tools/gen/source_batch25"
TMP="$ROOT/tools/gen/.tmp_batch25"
OUT="$ROOT/assets/sprites/signage"
...
for src in \
  NOTE_JobPosting.png \
  NOTE_RoomToLet.png \
  NOTE_HarbourBylaw.png \
  NOTE_EventFlyer.png \
  NOTE_FoundLost.png \
  NOTE_FerrySchedule.png
do
  if [[ ! -f "$SRC/$src" ]]; then
...
w, h = im.size
pad_x = max(6, round(w * 0.025))
pad_y = max(6, round(h * 0.025))
...
process_cutout NOTE_JobPosting
process_cutout NOTE_RoomToLet
process_cutout NOTE_HarbourBylaw
process_cutout NOTE_EventFlyer
process_cutout NOTE_FoundLost
process_cutout NOTE_FerrySchedule
