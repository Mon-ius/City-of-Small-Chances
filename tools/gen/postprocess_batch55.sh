...
SRC="$ROOT/tools/gen/source_batch55"
...
notes = {
    "PROP_Quay_LifeRing": "reads as a red-and-white cork lifebuoy on a backing-board, no legible text",
    "PROP_Quay_Ladder": "reads as a tall weathered quay ladder with grab-hoops, no legible text",
    "PROP_Quay_Fenders": "reads as a cluster of hung harbour fenders, no legible text",
}
...
for stem in stems:
    ...
    aspect = w / h
    if stem == "PROP_Quay_LifeRing" and not (0.72 <= aspect <= 1.32):
        raise SystemExit(f"{stem}: expected square-ish aspect, got {w}x{h}")
    if stem == "PROP_Quay_Ladder" and aspect >= 0.8:
        raise SystemExit(f"{stem}: expected tall portrait aspect, got {w}x{h}")
    if stem == "PROP_Quay_Fenders" and aspect >= 1.05:
