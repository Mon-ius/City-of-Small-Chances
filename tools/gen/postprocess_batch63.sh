...
SRC="$ROOT/tools/gen/source_batch63"
OUT="$ROOT/assets/sprites/fx"
GAIN="${SHIMMER_GAIN:-2.2}"   # luminance->alpha gain; lift the bright glints, fade the scatter

STEM="FX_Light_WaterShimmer"
...
# luminance -> alpha, gain curve: bright glints lift toward opaque, the scatter
...
cx, cy = w // 2, h // 2
ca = px[cx, cy][3]
# centre hue: should be neutral pale (so it tints cleanly). measure max channel spread
rs = gs = bs = n = 0
...
print(
    ...
    f"visual=broken glints of light shimmering on dark water, fades to transparent, no text"
)
...
if ca < 150:
    raise SystemExit(f"{stem}: shimmer centre too faint (alpha {ca}) — raise SHIMMER_GAIN")
if spread > 55:
    raise SystemExit(f"{stem}: centre not neutral (hue_spread {spread:.0f} > 55) — should be pale silver for tinting")
PY
