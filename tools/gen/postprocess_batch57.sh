...
SRC="$ROOT/tools/gen/source_batch57"
...
notes = {
    "PROP_Market_Cart": "reads as a two-wheeled costermonger's market handcart, no legible text",
    "PROP_Market_Crocks": "reads as a cluster of glazed earthenware market crocks, no legible text",
    "PROP_Market_Parasol": "reads as a tall canvas market parasol on a pole, no legible text",
}
...
for stem in stems:
    ...
    aspect = w / h
    if stem == "PROP_Market_Cart" and aspect <= 1.3:
        raise SystemExit(f"{stem}: expected landscape aspect, got {w}x{h}")
    if stem == "PROP_Market_Crocks" and not (0.7 <= aspect <= 1.4):
        raise SystemExit(f"{stem}: expected square-ish aspect, got {w}x{h}")
    if stem == "PROP_Market_Parasol" and aspect >= 1.05:
