...
SRC="$ROOT/tools/gen/source_batch54"
...
notes = {
    "PROP_Boat_Rowboat": "reads as a clinker rowing dory with shipped oars, no legible text",
    "PROP_Boat_Dinghy": "reads as a small sailing dinghy with a furled tan sail, no legible text",
    "PROP_Boat_Punt": "reads as a long flat-bottomed harbour punt with a quant pole, no legible text",
}
...
for stem in stems:
    ...
    aspect = w / h
    if stem == "PROP_Boat_Rowboat" and aspect <= 1.4:
        raise SystemExit(f"{stem}: expected landscape aspect, got {w}x{h}")
    if stem == "PROP_Boat_Dinghy" and not (0.75 <= aspect <= 1.35):
        raise SystemExit(f"{stem}: expected square-ish aspect, got {w}x{h}")
    if stem == "PROP_Boat_Punt" and aspect <= 1.9:
        raise SystemExit(f"{stem}: expected very-landscape aspect, got {w}x{h}")
PY
