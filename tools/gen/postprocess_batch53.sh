...
SRC="$ROOT/tools/gen/source_batch53"
...
for y in range(im.height):
    for x in range(im.width):
        ...
        if a < 10:
            px[x, y] = (0, 0, 0, 0)
            continue
        if g >= 120 and g - max(r, b) >= 18:
            neutral = min(max((r + b) // 2, 45), 190)
...
notes = {
    "PROP_Dock_Capstan": "reads as a squat oak warping capstan with bars and tarred rope coil, no legible text",
    "PROP_Dock_Derrick": "reads as a tall quayside derrick with block-and-tackle and empty hook, no legible text",
    "PROP_Dock_Timber": "reads as a low stack of sawn deals with dunnage and iron-banded chock, no legible text",
}

def green_fringe(r, g, b):
    return g >= 120 and g - max(r, b) >= 18
...
for stem in stems:
    ...
    print(
        ...
        f"corner_alpha={corners} residual_green_edge={fringe_pct:.2f}% "
    ...
    aspect = w / h
    if stem == "PROP_Dock_Capstan" and not (0.60 <= aspect <= 1.05):
        raise SystemExit(f"{stem}: expected portrait-ish aspect, got {w}x{h}")
    if stem == "PROP_Dock_Derrick" and not (0.25 <= aspect <= 0.75):
        raise SystemExit(f"{stem}: expected tall portrait aspect, got {w}x{h}")
    if stem == "PROP_Dock_Timber" and aspect <= 1.25:
