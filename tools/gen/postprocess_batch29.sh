#!/usr/bin/env bash
# Batch 29 — fx-007 shift-scene illustrations.
# Converts full-frame generated sources into 640x360 opaque RGB UI scenes.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
SRC="$ROOT/tools/gen/source_batch29"
OUT="$ROOT/assets/ui/shifts"
...
src = Path(sys.argv[1])
out = Path(sys.argv[2])
names = ("SHIFT_Labour", "SHIFT_Delivery", "SHIFT_Admin", "SHIFT_Service")
target = (640, 360)
...
def process(name):
    ...
    with Image.open(src_path) as im:
        ...
        rgb = rgb.filter(ImageFilter.GaussianBlur(1.4))
        quantized = rgb.quantize(colors=32, method=Image.Quantize.MEDIANCUT, dither=Image.Dither.NONE)
        rgb = quantized.convert("RGB")
        rgb = rgb.filter(ImageFilter.UnsharpMask(radius=0.7, percent=25, threshold=5))
