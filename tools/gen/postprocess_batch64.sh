...
SRC="$ROOT/tools/gen/source_batch64"
OUT="$ROOT/assets/sprites/fx"
GAIN="${BRAZIER_GAIN:-2.2}"   # luminance->alpha gain; lift the hot centre, fade the falloff

STEM="FX_Light_BrazierGlow"
...
# luminance -> alpha, gain curve: the hot centre lifts toward opaque, the ember
...
# quantise RGB (keep the hot orange), merge the smooth alpha back.
...
cx, cy = w // 2, h // 2
ca = px[cx, cy][3]
# hot-hue check at the bright centre (average a small patch)
rs = gs = bs = n = 0
...
print(
    ...
    f"visual=soft hot pool of firelight, white-gold core through orange to ember, fades to transparent, no text"
)
...
if ca < 160:
    raise SystemExit(f"{stem}: glow centre too faint (alpha {ca}) — raise BRAZIER_GAIN")
if not (rC >= gC >= bC and rC - bC > 30):
    raise SystemExit(f"{stem}: centre not hot/orange (R>G>B, R-B>30), got ({rC:.0f},{gC:.0f},{bC:.0f})")
aspect = w / h
if not (0.7 <= aspect <= 1.45):
    raise SystemExit(f"{stem}: expected round glow, got {w}x{h}")
PY
