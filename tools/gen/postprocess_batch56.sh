...
SRC="$ROOT/tools/gen/source_batch56"
...
# Neutralise any residual magenta/hot-pink fringe at the alpha edge.
px = im.load()
for y in range(im.height):
    for x in range(im.width):
        ...
        if a < 10:
            px[x, y] = (0, 0, 0, 0)
            continue
        if r >= 120 and b >= 120 and min(r, b) - g >= 18:
            neutral = min(max((r + b) // 2, 40), 200)
            px[x, y] = (min(r, neutral), g, min(b, neutral), a)
...
cpx = check.load()
cw, ch = check.size
corners = [cpx[0, 0][3], cpx[cw - 1, 0][3], cpx[0, ch - 1][3], cpx[cw - 1, ch - 1][3]]
if any(corners):
    raise SystemExit(f"{out}: expected alpha-0 corners after save, got {corners}")
PY
done
...
notes = {
    "PROP_Shop_Door": "reads as a weathered closed timber shop door with a stone step, no legible text",
    "PROP_Shop_Lantern": "reads as a wrought-iron bracket wall-lantern with warm amber glass, no legible text",
    "PROP_Shop_Crates": "reads as a tidy stack of retail crates of wares, no legible text",
}

def pink_fringe(r, g, b):
    return r >= 120 and b >= 120 and min(r, b) - g >= 18
...
for stem in stems:
    ...
    print(
        ...
        f"corner_alpha={corners} residual_pink_edge={fringe_pct:.2f}% "
        f"visible_pure_key={visible_key_pct:.2f}% bytes={byte_size} visual={notes[stem]}"
    )
    ...
    aspect = w / h
    if stem == "PROP_Shop_Door" and aspect >= 0.75:
        raise SystemExit(f"{stem}: expected tall portrait aspect, got {w}x{h}")
    if stem == "PROP_Shop_Lantern" and aspect >= 1.05:
        raise SystemExit(f"{stem}: expected portrait aspect, got {w}x{h}")
    if stem == "PROP_Shop_Crates" and not (0.7 <= aspect <= 1.45):
        raise SystemExit(f"{stem}: expected square-ish aspect, got {w}x{h}")
PY
