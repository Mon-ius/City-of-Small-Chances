...
SRC="$ROOT/tools/gen/source_batch59"
...
notes = {
    "PROP_Quay_CoalHeap": "reads as a black sea-coal heap with a shovel and hessian sacks, no legible text",
    "PROP_Quay_TarBarrel": "reads as an open pitch/tar barrel with a tar brush across the rim, no legible text",
    "PROP_Quay_SaltBarrel": "reads as an open barrel heaped with coarse sea-salt and a scoop, no legible text",
}
...
for stem in stems:
    ...
    aspect = w / h
    if stem == "PROP_Quay_CoalHeap" and aspect <= 1.05:
        raise SystemExit(f"{stem}: expected landscape aspect, got {w}x{h}")
    if stem == "PROP_Quay_TarBarrel" and aspect >= 1.1:
        raise SystemExit(f"{stem}: expected portrait/square aspect, got {w}x{h}")
    if stem == "PROP_Quay_SaltBarrel" and not (0.68 <= aspect <= 1.4):
