...
TMP="${TMPDIR:-/tmp}/harbour_batch35_postprocess"
SOURCE_DIR="${SOURCE_DIR:-${1:-$ROOT/tools/gen/source_batch35}}"
MAGICK="${MAGICK:-magick}"
CHROMA="${CODEX_HOME:-$HOME/.codex}/skills/.system/imagegen/scripts/remove_chroma_key.py"

NAMES=(Schoolmistress Knifegrinder TownCrier Widow)
TARGET_FEET_ROW=976
...
def is_green(r, g, b):
    return g >= 145 and r <= 130 and b <= 130 and (g - max(r, b)) >= 40

def is_magenta(r, g, b):
    return r >= 145 and b >= 145 and g <= 130 and (min(r, b) - g) >= 40
...
for y in range(im.height):
    for x in range(im.width):
        ...
        if a <= 8:
            px[x, y] = (0, 0, 0, 0)
            continue
        near_key = is_green(r, g, b) or is_magenta(r, g, b)
        if near_key:
            if a < 252:
                px[x, y] = (0, 0, 0, 0)
                removed += 1
...
names = ["Schoolmistress", "Knifegrinder", "TownCrier", "Widow"]
notes = {
    "Schoolmistress": "prim schoolmistress with blank slate, books, and pointer; no legible text",
    "Knifegrinder": "travelling knife-grinder with treadle grinding-wheel barrow; no legible text",
    "TownCrier": "tricorn town crier raising a brass handbell and holding a blank scroll; no legible text",
    "Widow": "mourning widow in black veil holding a small flower posy; no legible text",
}
failed = False
tiles = []
...
contact = Image.new("RGB", (len(tiles) * 180, 360), (70, 70, 70))
for i, tile in enumerate(tiles):
    contact.paste(tile, (i * 180, 0))
contact.save(tmp / "contact_batch35.png")
print(f"contact_sheet={tmp / 'contact_batch35.png'}")
