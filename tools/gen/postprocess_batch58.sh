...
SRC="$ROOT/tools/gen/source_batch58"
...
notes = {
    "PROP_Shop_Awning": "reads as a salt-faded striped shop awning with a scalloped valance, no legible text",
    "PROP_Shop_Window": "reads as a small-paned glazed shop window in a timber frame, no legible text",
    "PROP_Shop_FlowerBox": "reads as a timber window flower box of blooms and trailing foliage, no legible text",
}
...
for stem in stems:
    ...
    aspect = w / h
    if stem == "PROP_Shop_Awning" and aspect <= 1.3:
        raise SystemExit(f"{stem}: expected landscape aspect, got {w}x{h}")
    if stem == "PROP_Shop_Window" and not (0.66 <= aspect <= 1.12):
        raise SystemExit(f"{stem}: expected square-to-portrait aspect, got {w}x{h}")
    if stem == "PROP_Shop_FlowerBox" and aspect <= 1.6:
        raise SystemExit(f"{stem}: expected wide landscape aspect, got {w}x{h}")
PY
